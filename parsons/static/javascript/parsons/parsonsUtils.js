/* global createAlertBox hasFR hasMC multiAnswer verify */
    ...
    if (answeredQuestion) {
        ...
        answers.forEach((element, index) => {
            if (answers[index].includes("While") || answers[index].includes("For")) {
                let doInd = answers[index].indexOf(" do");
        ...
        for (let i = 0; i < answers.length; ++i) {
            totalAnswerLen += answers[i].length;
        }

        let studentCode = beginSet;
        for (let i = 0; i < answers.length; ++i) {
...
function getMulitConfirmFeedback(event, beginSet, endSet, confirms, comments) {
    ...
    if (answeredQuestion) {
        ...
        for (let i = 0; i < sortables.length; ++i) {
            answers.push("");
            for (let j = 0; j < sortables[i].childElementCount; ++j) {
                if (sortables[i].childNodes[j].innerText != undefined) {
                    ...
                    if (sortables[i].childNodes[j].innerText.includes("While") || sortables[i].childNodes[j].innerText.includes("For")) {
                        let doInd = sortables[i].childNodes[j].innerText.indexOf(" do");
...
function colorCodeText(divID) {
    ...
    for (var i = 0; i < lines.length; ++i) {
        ...
        if (divID == "firstSetCode") {
            ...
            if (numSpaces != 0) {
                parsonsSpaces = "";
                for (let spaces = 0; spaces < numSpaces; ++spaces) {
                    parsonsSpaces += "&nbsp";
                }
            }
        }
